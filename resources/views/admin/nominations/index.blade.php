...
@section('title', 'Nominations Dashboard - Admin')
...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Header -->
        <div class="mb-8">
            <a href="{{ route('admin.dashboard') }}" class="inline-flex items-center text-purple-600 hover:text-purple-700 font-medium mb-4 transition">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="text-3xl font-bold text-blue-600">{{ $stats['under_review'] }}</div>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
                <div class="text-sm font-medium text-gray-600 mb-1">Shortlisted</div>
                <div class="text-3xl font-bold text-purple-600">{{ $stats['shortlisted'] }}</div>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="text-3xl font-bold text-green-600">{{ $stats['winner'] }}</div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow mb-6">
            <div class="p-6">
                <form method="GET" action="{{ route('admin.nominations.index') }}" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div class="flex items-end gap-2">
                            ...
                            <a href="{{ route('admin.nominations.index') }}" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 font-medium rounded-lg transition">
                                Reset
                            </a>
                        </div>
                    </div>
                ...
                <div class="mt-4 pt-4 border-t border-gray-200 flex justify-between items-center">
                    <form method="POST" action="{{ route('admin.nominations.bulk-action') }}" id="bulkActionForm" class="flex items-center gap-3">
                        @csrf
                    ...
                    <a href="{{ route('admin.nominations.export', request()->query()) }}" 
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4">
                                <input type="checkbox" name="nominations[]" value="{{ $nomination->id }}" form="bulkActionForm" class="nomination-checkbox w-4 h-4 text-purple-600 border-gray-300 rounded focus:ring-purple-500">
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                #{{ $nomination->id }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">{{ $nomination->nominee_full_name }}</div>
                                <div class="text-sm text-gray-500">{{ $nomination->nominee_email }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900">{{ $nomination->award_category }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="{{ route('admin.nominations.show', $nomination) }}" 
                                   class="text-purple-600 hover:text-purple-900 mr-3">View</a>
                                <form action="{{ route('admin.nominations.destroy', $nomination) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
